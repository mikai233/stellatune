...
Controller 先完成回滚和过期请求检查，再上报错误。全局提示只包含本地化短文案与“查看日志”。取消和后台重试可静默记录；普通 ERROR 日志不会自动弹提示。相同类别、根因和上下文在两秒内合并通知，命令与事件的日志记录仍分别保留。输出切换期间的异步设备错误由命令结果统一通知，保留原输出恢复与失败设备置灰。

## 全屏日志页

- 播放栏日志图标常驻，带未读错误数；设置页与启动失败页也有入口。
