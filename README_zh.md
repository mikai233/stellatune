...
- 🚀 **卓越的跨平台能力**：从底层开始为真正的跨平台而生。在桌面端（Windows、macOS、Linux）和移动端都能享受流畅、响应迅速且具有原生体验的 UI，且所有平台统一运行在稳定强健的 Rust 核心之上。
- 🧩 **强大的插件生态系统**：无限的扩展可能。StellaTune 拥有极其灵活的 WebAssembly (Wasm) 与原生插件系统。您可以通过放入自定义的音频源、解码器、歌词服务、DSP 以及输出端插件，来将播放器定制成您理想的模样。
- 🗂️ **现代化的本地音乐库**：为现代音乐爱好者精心打造，美观与实用并存的用户体验，让您能轻松构建、整理和欣赏您的本地收藏。
