...
impl SinkWorker {
    ...
    fn call_control(
        ...
        self.ctrl_tx.send(constructor(resp_tx)).map_err(|_| {
            PipelineError::StageFailure("sink loop control channel disconnected".to_string())
        })?;
        resp_rx.recv_timeout(timeout).map_err(|error| match error {
            ...
            RecvTimeoutError::Disconnected => {
                PipelineError::StageFailure("sink loop dropped control response".to_string())
            },
        })?
    }
}
