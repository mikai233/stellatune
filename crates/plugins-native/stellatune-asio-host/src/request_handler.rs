...
fn handle_open<W: Write>(
    ...
    eprintln!(
        "asio host request Open begin: device={} session={} spec={}Hz/{}ch buffer_size_frames={:?}",
        device_id, selection_session_id, spec.sample_rate, spec.channels, buffer_size_frames
    );
    ...
    match validate_selection_session(state, &selection_session_id, &device_id) {
        Ok(()) => {
            ...
            match StreamState::open(&device_id, spec, buffer_size_frames, shared_ring) {
                Ok(next_state) => {
                    eprintln!(
