...
use super::super::{
    DisruptFadeKind, EngineState, Event, ManualSwitchTiming, PlayerState, SessionStopMode,
    SharedTrackInfo, emit_position_event, ensure_output_spec_prewarm,
    flush_pending_plugin_disables, maybe_fade_out_before_disrupt, next_position_session_id,
    set_state, stop_decode_session, track_ref_to_engine_token, track_ref_to_event_path,
};
...
fn load_track_ref(
    ...
    if let Err(message) = flush_pending_plugin_disables(state, events) {
...
impl Handler<SwitchTrackRefMessage> for ControlActor {
    fn handle(
        ...
        if let Err(err) = load_track_ref(state, events, track_info, message.track) {
