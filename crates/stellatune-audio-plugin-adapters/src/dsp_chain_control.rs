...
use crate::dsp_runtime::DspWorkerSpec;
...
pub struct DspChainEntry {
    pub stage: DspChainStage,
    pub spec: DspWorkerSpec,
}
...
pub fn parse_stage_control_payload(
    ...
    revision: u64,
    payload: &[u8],
) -> Result<Option<Vec<DspChainEntry>>, String> {
    ...
    entries.extend(parsed.pre_mix.into_iter().map(|item| DspChainEntry {
    ...
    entries.extend(parsed.post_mix.into_iter().map(|item| DspChainEntry {
...
mod tests {
    ...
    #[test]
    fn parses_v1_payload_into_staged_specs() {
        let payload = br#"{
            "pre_mix":[{"plugin_id":"a","type_id":"eq","config_json":"{}"}],
            "post_mix":[{"plugin_id":"b","type_id":"limiter","config_json":"{}"}]
        }"#;
        ...
        assert_eq!(entries[0].spec.plugin_id, "a");
        assert_eq!(entries[1].stage, DspChainStage::PostMix);
        assert_eq!(entries[1].spec.type_id, "limiter");
    }

    #[test]
    fn returns_none_for_unrelated_schema() {
        let payload = br#"{"pre_mix":[],"post_mix":[]}"#;
        let parsed = parse_stage_control_payload("other.schema/v1", 1, payload)
